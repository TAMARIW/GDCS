...
void _dwt_enableclocks(uint8_t spi_num, int clocks);
// Configure the ucode (FP algorithm) parameters
void _dwt_configlde(uint8_t spi_num, int prf);
// Load ucode from OTP/ROM
void _dwt_loaducodefromrom(uint8_t spi_num);
// Read non-volatile memory
uint32_t _dwt_otpread(uint8_t spi_num, uint32_t address);
// Program the non-volatile memory
uint32_t _dwt_otpprogword32(uint8_t spi_num, uint32_t data, uint16_t address);
// Upload the device configuration into always on memory
void _dwt_aonarrayupload(uint8_t spi_num);
// -------------------------------------------------------------------------------------------------------------------
...
int dwt_initialise(uint8_t spi_num, uint16_t config) {
    ...
    dw1000local.deviceID = dwt_readdevid(spi_num);
    if (DWT_DEVICE_ID != dw1000local.deviceID)  // MP IC ONLY (i.e. DW1000) FOR THIS CODE
    {
        return DWT_ERROR;
    }

    _dwt_enableclocks(spi_num, FORCE_SYS_XTI);  // NOTE: set system clock to XTI - this is necessary to make sure the values read by _dwt_otpread are reliable

    // Configure the CPLL lock detect
    dwt_writetodevice(spi_num, EXT_SYNC_ID, EC_CTRL_OFFSET, 1, &plllockdetect);

    // Read OTP revision number
    otp_addr = _dwt_otpread(spi_num, XTRIM_ADDRESS) & 0xffff;  // Read 32 bit value, XTAL trim val is in low octet-0 (5 bits)
    dw1000local.otprev = (otp_addr >> 8) & 0xff;      // OTP revision is next byte

    // Load LDO tune from OTP and kick it if there is a value actually programmed.
    ldo_tune = _dwt_otpread(spi_num, LDOTUNE_ADDRESS);
    if ((ldo_tune & 0xFF) != 0) {
        uint8_t ldok = OTP_SF_LDO_KICK;
        // Kick LDO tune
        dwt_writetodevice(spi_num, OTP_IF_ID, OTP_SF, 1, &ldok);  // Set load LDE kick bit
        dw1000local.sleep_mode |= AON_WCFG_ONW_LLDO;     // LDO tune must be kicked at wake-up
    }

    // Load Part and Lot ID from OTP
    dw1000local.partID = _dwt_otpread(spi_num, PARTID_ADDRESS);
    dw1000local.lotID = _dwt_otpread(spi_num, LOTID_ADDRESS);
    ...
    // Configure XTAL trim
    dwt_xtaltrim(spi_num, dw1000local.xtrim);

    // Load leading edge detect code
    if (config & DWT_LOADUCODE) {
        _dwt_loaducodefromrom(spi_num);
        dw1000local.sleep_mode |= AON_WCFG_ONW_LLDE;  // microcode must be loaded at wake-up
    } else                                            // Should disable the LDERUN enable bit in 0x36, 0x4
    {
        uint16_t rega = dwt_read16bitoffsetreg(spi_num, PMSC_ID, PMSC_CTRL1_OFFSET + 1);
        rega &= 0xFDFF;  // Clear LDERUN bit
        dwt_write16bitoffsetreg(spi_num, PMSC_ID, PMSC_CTRL1_OFFSET + 1, rega);
    }

    _dwt_enableclocks(spi_num, ENABLE_ALL_SEQ);  // Enable clocks for sequencing

    // Read system register / store local copy
    dw1000local.sysCFGreg = dwt_read32bitreg(spi_num, SYS_CFG_ID);  // Read sysconfig register
 ...
 * no return value
 */
void dwt_setGPIOforEXTTRX(uint8_t spi_num) {
    uint8_t buf[GPIO_MODE_LEN];

    // Set the GPIO to control external PA/LNA
    dwt_readfromdevice(spi_num, GPIO_CTRL_ID, GPIO_MODE_OFFSET, GPIO_MODE_LEN, buf);

    buf[GPIO_LNA_BYTE_NUM] |= (GPIO_PIN5_EXTTXE_8 + GPIO_PIN6_EXTRXE_8);

    dwt_writetodevice(spi_num, GPIO_CTRL_ID, GPIO_MODE_OFFSET, GPIO_MODE_LEN, buf);
}
 ...
 *
 * input parameters
 * @param spi_num    -   ID of the spi interface (equal to the used DW1000 device entity)
 ...
 * no return value
 */
void dwt_setGPIOdirection(uint8_t spi_num, uint32_t gpioNum, uint32_t direction) {
    ...
    dwt_writetodevice(spi_num, GPIO_CTRL_ID, GPIO_DIR_OFFSET, GPIO_DIR_LEN, buf);
}
 ...
 * no return value
 */
void dwt_setGPIOvalue(uint8_t spi_num, uint32_t gpioNum, uint32_t value) {
    ...
    dwt_writetodevice(spi_num, GPIO_CTRL_ID, GPIO_DOUT_OFFSET, GPIO_DOUT_LEN, buf);
}
 ...
 *
 * input parameters
 * @param spi_num - ID of the spi interface (equal to the used DW1000 device entity) ( 0 / 1 )
 *
 * output parameters
 *
 * returns the read value which for DW1000 is 0xDECA0130
 */
uint32_t dwt_readdevid(uint8_t spi_num) { return dwt_read32bitoffsetreg(spi_num, DEV_ID_ID, 0); }
 ...
 * no return value
 */
void dwt_configuretxrf(uint8_t spi_num, dwt_txconfig_t *config) {
    // Configure RF TX PG_DELAY
    dwt_writetodevice(spi_num, TX_CAL_ID, TC_PGDELAY_OFFSET, 1, &config->PGdly);

    // Configure TX power
    dwt_write32bitreg(spi_num, TX_POWER_ID, config->power);
}
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_configure(uint8_t spi_num, dwt_config_t *config) {
    ...
    dwt_write32bitreg(spi_num, SYS_CFG_ID, dw1000local.sysCFGreg);
    // Set the lde_replicaCoeff
    dwt_write16bitoffsetreg(spi_num, LDE_IF_ID, LDE_REPC_OFFSET, reg16);

    _dwt_configlde(spi_num, prfIndex);

    // Configure PLL2/RF PLL block CFG (for a given channel)
    dwt_writetodevice(spi_num, FS_CTRL_ID, FS_PLLCFG_OFFSET, 5, &pll2_config[chan_idx[chan]][0]);

    // Configure RF RX blocks (for specified channel/bandwidth)
    dwt_writetodevice(spi_num, RF_CONF_ID, RF_RXCTRLH_OFFSET, 1, &rx_config[bw]);

    // Configure RF TX blocks (for specified channel and PRF)
    // Configure RF TX control
    dwt_write32bitoffsetreg(spi_num, RF_CONF_ID, RF_TXCTRL_OFFSET, tx_config[chan_idx[chan]]);

    // Configure the baseband parameters (for specified PRF, bit rate, PAC, and SFD settings)
    // DTUNE0
    dwt_write16bitoffsetreg(spi_num, DRX_CONF_ID, DRX_TUNE0b_OFFSET, sftsh[config->dataRate][config->nsSFD]);

    // DTUNE1
    dwt_write16bitoffsetreg(spi_num, DRX_CONF_ID, DRX_TUNE1a_OFFSET, dtune1[prfIndex]);

    if (config->dataRate == DWT_BR_110K) {
        dwt_write16bitoffsetreg(spi_num, DRX_CONF_ID, DRX_TUNE1b_OFFSET, 0x64);
    } else {
        if (config->txPreambLength == DWT_PLEN_64) {
            uint8_t temp = 0x10;
            dwt_write16bitoffsetreg(spi_num, DRX_CONF_ID, DRX_TUNE1b_OFFSET, 0x10);
            dwt_writetodevice(spi_num, DRX_CONF_ID, 0x26, 1, &temp);
        } else {
            uint8_t temp = 0x28;
            dwt_write16bitoffsetreg(spi_num, DRX_CONF_ID, DRX_TUNE1b_OFFSET, 0x20);
            dwt_writetodevice(spi_num, DRX_CONF_ID, 0x26, 1, &temp);
        }
    }

    // DTUNE2
    dwt_write32bitoffsetreg(spi_num, DRX_CONF_ID, DRX_TUNE2_OFFSET, digital_bb_config[prfIndex][config->rxPAC]);
    ...
    dwt_write16bitoffsetreg(spi_num, DRX_CONF_ID, DRX_SFDTOC_OFFSET, config->sfdTO);

    // Configure AGC parameters
    dwt_write32bitoffsetreg(spi_num, AGC_CFG_STS_ID, 0xC, agc_config.lo32);
    dwt_write16bitoffsetreg(spi_num, AGC_CFG_STS_ID, 0x4, agc_config.target[prfIndex]);

    // Set (non-standard) user SFD for improved performance,
    if (config->nsSFD) {
        // Write non standard (DW) SFD length
        dwt_writetodevice(spi_num, USR_SFD_ID, 0x00, 1, &dwnsSFDlen[config->dataRate]);
        nsSfd_result = 3;
        useDWnsSFD = 1;
    }
    ...
    dwt_write32bitreg(spi_num, CHAN_CTRL_ID, regval);
    ...
    dwt_write32bitoffsetreg(spi_num, TX_FCTRL_ID, 0, dw1000local.txFCTRL);
 ...
 * no return value
 */
void dwt_setrxantennadelay(uint8_t spi_num, uint16_t rxDelay) {
    // Set the RX antenna delay for auto TX timestamp adjustment
    dwt_write16bitoffsetreg(spi_num, LDE_IF_ID, LDE_RXANTD_OFFSET, rxDelay);
}
 ...
 * no return value
 */
void dwt_settxantennadelay(uint8_t spi_num, uint16_t txDelay) {
    // Set the TX antenna delay for auto TX timestamp adjustment
    dwt_write16bitoffsetreg(spi_num, TX_ANTD_ID, 0x0, txDelay);
}
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_writetxdata(uint8_t spi_num, uint16_t txFrameLength, uint8_t *txFrameBytes, uint16_t txBufferOffset) {
#ifdef DWT_API_ERROR_CHECK
    ...
    dwt_writetodevice(spi_num, TX_BUFFER_ID, txBufferOffset, txFrameLength - 2, txFrameBytes);
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_writetxfctrl(uint8_t spi_num, uint16_t txFrameLength, uint16_t txBufferOffset) {
#ifdef DWT_API_ERROR_CHECK
    if (dw1000local.longFrames) {
        if (txFrameLength > 1023) {
            return DWT_ERROR;
    ...
    dwt_write32bitoffsetreg(spi_num, TX_FCTRL_ID, 0, reg32);
 ...
 * no return value
 */
void dwt_readrxdata(uint8_t spi_num, uint8_t *buffer, uint16_t length, uint16_t rxBufferOffset) { dwt_readfromdevice(spi_num, RX_BUFFER_ID, rxBufferOffset, length, buffer); }
 ...
 * no return value
 */
void dwt_readaccdata(uint8_t spi_num, uint8_t *buffer, uint16_t len, uint16_t accOffset) {
    // Force on the ACC clocks if we are sequenced
    _dwt_enableclocks(spi_num, READ_ACC_ON);

    dwt_readfromdevice(spi_num, ACC_MEM_ID, accOffset, len, buffer);

    _dwt_enableclocks(spi_num, READ_ACC_OFF);  // Revert clocks back
}
 ...
 * no return value
 */
void dwt_readdiagnostics(uint8_t spi_num, dwt_rxdiag_t *diagnostics) {
    // Read the HW FP index
    diagnostics->firstPath = dwt_read16bitoffsetreg(spi_num, RX_TIME_ID, RX_TIME_FP_INDEX_OFFSET);

    // LDE diagnostic data
    diagnostics->maxNoise = dwt_read16bitoffsetreg(spi_num, LDE_IF_ID, LDE_THRESH_OFFSET);

    dwt_readfromdevice(spi_num, RX_FQUAL_ID, 0x0, 2, (uint8_t *)&diagnostics->stdNoise);
    dwt_readfromdevice(spi_num, RX_FQUAL_ID, 0x2, 2, (uint8_t *)&diagnostics->firstPathAmp2);
    dwt_readfromdevice(spi_num, RX_FQUAL_ID, 0x4, 2, (uint8_t *)&diagnostics->firstPathAmp3);
    dwt_readfromdevice(spi_num, RX_FQUAL_ID, 0x6, 2, (uint8_t *)&diagnostics->maxGrowthCIR);

    diagnostics->firstPathAmp1 = dwt_read16bitoffsetreg(spi_num, RX_TIME_ID, RX_TIME_FP_AMPL1_OFFSET);

    diagnostics->rxPreamCount = (dwt_read32bitreg(spi_num, RX_FINFO_ID) & RX_FINFO_RXPACC_MASK) >> RX_FINFO_RXPACC_SHIFT;
}
 ...
 * no return value
 */
void dwt_readtxtimestamp(uint8_t spi_num, uint8_t *timestamp) {
    dwt_readfromdevice(spi_num, TX_TIME_ID, 0, TX_TIME_TX_STAMP_LEN, timestamp);  // Read bytes directly into buffer
}
 ...
 */
uint32_t dwt_readtxtimestamphi32(uint8_t spi_num) { return dwt_read32bitoffsetreg(spi_num, TX_TIME_ID, 1); }
 ...
 */
uint32_t dwt_readtxtimestamplo32(uint8_t spi_num) { return dwt_read32bitoffsetreg(spi_num, TX_TIME_ID, 0); }
 ...
 * no return value
 */
void dwt_readrxtimestamp(uint8_t spi_num, uint8_t *timestamp) {
    dwt_readfromdevice(spi_num, RX_TIME_ID, 0, RX_TIME_RX_STAMP_LEN, timestamp);  // Get the adjusted time of arrival
}
 ...
 */
uint32_t dwt_readrxtimestamphi32(uint8_t spi_num) { return dwt_read32bitoffsetreg(spi_num, RX_TIME_ID, 1); }
 ...
 */
uint32_t dwt_readrxtimestamplo32(uint8_t spi_num) { return dwt_read32bitoffsetreg(spi_num, RX_TIME_ID, 0); }
 ...
 */
uint32_t dwt_readsystimestamphi32(uint8_t spi_num) { return dwt_read32bitoffsetreg(spi_num, SYS_TIME_ID, 1); }
 ...
 * no return value
 */
void dwt_readsystime(uint8_t spi_num, uint8_t *timestamp) { dwt_readfromdevice(spi_num, SYS_TIME_ID, 0, SYS_TIME_LEN, timestamp); }
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_writetodevice(uint8_t spi_num, uint16_t recordNumber, uint16_t index, uint32_t length, const uint8_t *buffer) {
    ...
    // Write it to the SPI
    return writetospi(spi_num, cnt, header, length, buffer);
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_readfromdevice(uint8_t spi_num, uint16_t recordNumber, uint16_t index, uint32_t length, uint8_t *buffer) {
    uint8_t header[3];  // Buffer to compose header in
    int cnt = 0;        // Counter for length of header
#ifdef DWT_API_ERROR_CHECK
    if (recordNumber > 0x3F) {
    ...
    return readfromspi(spi_num, cnt, header, length, buffer);  // result is stored in the buffer
 ...
 */
uint32_t dwt_read32bitoffsetreg(uint8_t spi_num, int regFileID, int regOffset) {
    uint32_t regval = DWT_ERROR;
    int j;
    uint8_t buffer[4];

    int result = dwt_readfromdevice(spi_num, regFileID, regOffset, 4, buffer);  // Read 4 bytes (32-bits) register into buffer
 ...
 */
uint16_t dwt_read16bitoffsetreg(uint8_t spi_num, int regFileID, int regOffset) {
    uint16_t regval = DWT_ERROR;
    uint8_t buffer[2];

    int result = dwt_readfromdevice(spi_num, regFileID, regOffset, 2, buffer);  // Read 2 bytes (16-bits) register into buffer
 ...
 *
 * input parameters:
 * @param spi_num   - ID of the used spi interface (equal to the DW1000 device entity) ( 0 / 1 )
 * @param regFileID - ID of register file or buffer being accessed
 * @param regOffset - the index into register file or buffer being accessed
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_write16bitoffsetreg(uint8_t spi_num, int regFileID, int regOffset, uint16_t regval) {
    int reg;
    uint8_t buffer[2];
    ...
    reg = dwt_writetodevice(spi_num, regFileID, regOffset, 2, buffer);
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_write32bitoffsetreg(uint8_t spi_num, int regFileID, int regOffset, uint32_t regval) {
    int j;
    int reg;
    uint8_t buffer[4];
    ...
    reg = dwt_writetodevice(spi_num, regFileID, regOffset, 4, buffer);
 ...
 * no return value
 */
void dwt_enableframefilter(uint8_t spi_num, uint16_t enable) {
    uint32_t sysconfig = SYS_CFG_MASK & dwt_read32bitreg(spi_num, SYS_CFG_ID);  // Read sysconfig register
    ...
    dwt_write32bitreg(spi_num, SYS_CFG_ID, sysconfig);
}
 ...
 * no return value
 */
void dwt_setpanid(uint8_t spi_num, uint16_t panID) {
    // PAN ID is high 16 bits of register
    dwt_write16bitoffsetreg(spi_num, PANADR_ID, 2, panID);
}
 ...
 * no return value
 */
void dwt_setaddress16(uint8_t spi_num, uint16_t shortAddress) {
    // Short address into low 16 bits
    dwt_write16bitoffsetreg(spi_num, PANADR_ID, 0, shortAddress);
}
 ...
 * no return value
 */
void dwt_seteui(uint8_t spi_num, uint8_t *eui64) { dwt_writetodevice(spi_num, EUI_64_ID, 0x0, 8, eui64); }
 ...
 * no return value
 */
void dwt_geteui(uint8_t spi_num, uint8_t *eui64) { dwt_readfromdevice(spi_num, EUI_64_ID, 0x0, 8, eui64); }
 ...
 * no return value
 */
void dwt_otpread(uint8_t spi_num, uint32_t address, uint32_t *array, uint8_t length) {
    int i;

    _dwt_enableclocks(spi_num, FORCE_SYS_XTI);  // NOTE: Set system clock to XTAL - this is necessary to make sure the values read by _dwt_otpread are reliable

    for (i = 0; i < length; i++) {
        array[i] = _dwt_otpread(spi_num, address + i);
    }

    _dwt_enableclocks(spi_num, ENABLE_ALL_SEQ);  // Restore system clock to PLL
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
uint32_t _dwt_otpsetmrregs(uint8_t spi_num, int mode) {
    uint8_t rd_buf[4];
    uint8_t wr_buf[4];
    ...
    // Set MRA, MODE_SEL
    wr_buf[0] = 0x03;
    dwt_writetodevice(spi_num, OTP_IF_ID, OTP_CTRL + 1, 1, wr_buf);
    ...
    dwt_writetodevice(spi_num, OTP_IF_ID, OTP_WDAT, 2, wr_buf);

    // Set WRITE_MR
    wr_buf[0] = 0x08;
    dwt_writetodevice(spi_num, OTP_IF_ID, OTP_CTRL, 1, wr_buf);
    ...
    if (((mode & 0x0f) == 0x1) || ((mode & 0x0f) == 0x2)) {
        ...
        dwt_readfromdevice(spi_num, OTP_IF_ID, OTP_STAT, 1, rd_buf);
    }
...
uint32_t _dwt_otpprogword32(uint8_t spi_num, uint32_t data, uint16_t address) {
    ...
    wr_buf[0] = data & 0xff;
    dwt_writetodevice(spi_num, OTP_IF_ID, OTP_WDAT, 4, wr_buf);

    // Write the address [10:0]
    wr_buf[1] = (address >> 8) & 0x07;
    wr_buf[0] = address & 0xff;
    dwt_writetodevice(spi_num, OTP_IF_ID, OTP_ADDR, 2, wr_buf);
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
uint32_t dwt_otpwriteandverify(uint8_t spi_num, uint32_t value, uint16_t address) {
    int prog_ok = DWT_SUCCESS;
    int retry = 0;
    // Firstly set the system clock to crystal
    _dwt_enableclocks(spi_num, FORCE_SYS_XTI);  // set system clock to XTI
    ...
    _dwt_otpsetmrregs(spi_num, 1);  // Set mode for programming

    // For each value to program - the readback/check is done couple of times to verify it has programmed successfully
    while (1) {
        _dwt_otpprogword32(spi_num, value, address);

        if (_dwt_otpread(spi_num, address) == value) {
            break;
        }
        retry++;
        if (retry == 5) {
    ...
    _dwt_otpsetmrregs(spi_num, 4);  // Set mode for reading

    if (_dwt_otpread(spi_num, address) != value)  // If this does not pass please check voltage supply on VDDIO
    {
        prog_ok = DWT_ERROR;
    }

    _dwt_otpsetmrregs(spi_num, 0);  // Setting OTP mode register for low RM read - resetting the device would be alternative
 ...
 * no return value
 */
void _dwt_aonconfigupload(uint8_t spi_num) {
    uint8_t buf[1];

    buf[0] = 0x04;
    dwt_writetodevice(spi_num, AON_ID, AON_CTRL_OFFSET, 1, buf);
    buf[0] = 0x00;
    dwt_writetodevice(spi_num, AON_ID, AON_CTRL_OFFSET, 1, buf);
}
 ...
 * no return value
 */
void dwt_entersleep(uint8_t spi_num) {
    // Copy config to AON - upload the new configuration
    _dwt_aonarrayupload(spi_num);
}
 ...
 * no return value
 */
void dwt_configuresleepcnt(uint8_t spi_num, uint16_t sleepcnt) {
    uint8_t buf[2];

    buf[0] = 0x01;
    dwt_writetodevice(spi_num, PMSC_ID, PMSC_CTRL0_OFFSET, 1, buf);

    buf[0] = 0;
    dwt_writetodevice(spi_num, AON_ID, AON_CFG0_OFFSET, 1, buf);  // To make sure we don't accidentaly go to sleep

    buf[0] = 0;
    dwt_writetodevice(spi_num, AON_ID, AON_CFG1_OFFSET, 1, buf);
    // Disable the sleep counter
    _dwt_aonconfigupload(spi_num);

    // Set new value
    buf[0] = sleepcnt & 0xFF;
    buf[1] = (sleepcnt >> 8) & 0xFF;
    dwt_writetodevice(spi_num, AON_ID, (AON_CFG0_OFFSET + 2), 2, buf);
    _dwt_aonconfigupload(spi_num);
 ...
 */
uint16_t dwt_calibratesleepcnt(uint8_t spi_num) {
    uint8_t buf[2];
    uint16_t result;
    ...
    // Set up address
    buf[0] = 118;
    dwt_writetodevice(spi_num, AON_ID, AON_ADDR_OFFSET, 1, buf);
    ...
    dwt_readfromdevice(spi_num, AON_ID, AON_RDAT_OFFSET, 1, buf);
    result = buf[0];
    result = result << 8;
 ...
 * no return value
 */
void dwt_configuresleep(uint8_t spi_num, uint16_t mode, uint8_t wake) {
    uint8_t buf[1];

    // Add predefined sleep settings before writing the mode
    mode |= dw1000local.sleep_mode;
    dwt_write16bitoffsetreg(spi_num, AON_ID, AON_WCFG_OFFSET, mode);

    buf[0] = wake;

    dwt_writetodevice(spi_num, AON_ID, AON_CFG0_OFFSET, 1, buf);
}
 ...
 * no return value
 */
void dwt_entersleepaftertx(uint8_t spi_num, int enable) {
    uint32_t reg = dwt_read32bitoffsetreg(spi_num, PMSC_ID, PMSC_CTRL1_OFFSET);
    ...
    dwt_write32bitoffsetreg(spi_num, PMSC_ID, PMSC_CTRL1_OFFSET, reg);
}
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_spicswakeup(uint8_t spi_num, uint8_t *buff, uint16_t length) {
    if (dwt_readdevid(spi_num) != DWT_DEVICE_ID)  // Device was in deep sleep (the first read fails)
    {
        // Need to keep chip select line low for at least 500us
        dwt_readfromdevice(spi_num, 0x0, 0x0, length, buff);  // Do a long read to wake up the chip (hold the chip select low)
    ...
    if (dwt_readdevid(spi_num) != DWT_DEVICE_ID) {
        return DWT_ERROR;
    }
 ...
 * no return value
 */
void _dwt_configlde(uint8_t spi_num, int prfIndex) {
    uint8_t x = LDE_PARAM1;

    dwt_writetodevice(spi_num, LDE_IF_ID, LDE_CFG1_OFFSET, 1, &x);  // 8-bit configuration register

    if (prfIndex) {
        dwt_write16bitoffsetreg(spi_num, LDE_IF_ID, LDE_CFG2_OFFSET, (uint16_t)LDE_PARAM3_64);  // 16-bit LDE configuration tuning register
    } else {
        dwt_write16bitoffsetreg(spi_num, LDE_IF_ID, LDE_CFG2_OFFSET, (uint16_t)LDE_PARAM3_16);
    }
}
...
void _dwt_loaducodefromrom(uint8_t spi_num) {
    ...
    wr_buf[1] = 0x03;
    wr_buf[0] = 0x01;
    dwt_writetodevice(spi_num, PMSC_ID, PMSC_CTRL0_OFFSET, 2, wr_buf);
    // Kick off the LDE load
    dwt_write16bitoffsetreg(spi_num, OTP_IF_ID, OTP_CTRL, OTP_CTRL_LDELOAD);  // Set load LDE kick bit
 ...
 * no return value
 */
void dwt_loadopsettabfromotp(uint8_t spi_num, uint8_t gtab_sel) {
    uint8_t wr_buf[2];
    ...
    dwt_write16bitoffsetreg(spi_num, OTP_IF_ID, OTP_SF, reg);  // Set load gtab kick bit (bit0) and gtab selection bit
 ...
 * no return value
 */
void dwt_setsmarttxpower(uint8_t spi_num, int enable) {
 ...
 * no return value
 */
void dwt_enableautoack(uint8_t spi_num, uint8_t responseDelayTime) {
    // Set auto ACK reply delay
    dwt_write16bitoffsetreg(spi_num, ACK_RESP_T_ID, 0x2, (responseDelayTime << 8));  // in symbols
    // Enable auto ACK
 ...
 * no return value
 */
void dwt_setdblrxbuffmode(uint8_t spi_num, int enable) {
    if (enable) {
 ...
 * no return value
 */
void dwt_setautorxreenable(uint8_t spi_num, int enable) {
    uint8_t byte = 0;
    ...
    dwt_writetodevice(spi_num, SYS_CFG_ID, 3, 1, &byte);
}
 ...
 * no return value
 */
void dwt_setrxaftertxdelay(uint8_t spi_num, uint32_t rxDelayTime) {
    uint32_t val = dwt_read32bitreg(spi_num, ACK_RESP_T_ID);  // Read ACK_RESP_T_ID register

    val &= ~(ACK_RESP_T_W4R_TIM_MASK);  // Clear the timer (19:0)

    val |= (rxDelayTime & ACK_RESP_T_W4R_TIM_MASK);  // In UWB microseconds (e.g. turn the receiver on 20uus after TX)

    dwt_write32bitreg(spi_num, ACK_RESP_T_ID, val);
}
 ...
 */
uint8_t dwt_checkIRQ(uint8_t spi_num) {
    uint8_t temp;

    dwt_readfromdevice(spi_num, SYS_STATUS_ID, 0, 1, &temp);
 ...
 * no return value
 */
void dwt_isr(uint8_t spi_num)  // Assume interrupt can supply context
{
    uint32_t status = 0;
    ...
    status = dw1000local.cdata.status = dwt_read32bitreg(spi_num, SYS_STATUS_ID);  // Read status register low 32bits
    ...
    if ((status & SYS_STATUS_LDEDONE) && (dw1000local.dblbuffon == 0)) {
        if ((status & (SYS_STATUS_LDEDONE | SYS_STATUS_RXPHD | SYS_STATUS_RXSFDD)) != (SYS_STATUS_LDEDONE | SYS_STATUS_RXPHD | SYS_STATUS_RXSFDD)) {
            ...
            dwt_forcetrxoff(spi_num);  // this will clear all events
            ...
            if (dw1000local.sysCFGreg & SYS_CFG_RXAUTR) {
                dwt_write16bitoffsetreg(spi_num, SYS_CTRL_ID, 0, (uint16_t)SYS_CTRL_RXENAB);
            } else {
    ...
    if (status & SYS_STATUS_RXFCG)  // Receiver FCS Good
    {
        if (status & SYS_STATUS_LDEDONE)  // LDE done/finished
        {
            ...
            {
                len = dwt_read16bitoffsetreg(spi_num, RX_FINFO_ID, 0) & 0x3FF;
                dwt_readfromdevice(spi_num, RX_BUFFER_ID, 0, 2, dw1000local.cdata.fctrl);
                ...
                if (dw1000local.dblbuffon == 0)  // If no double buffering
                {
                    ...
                    dwt_write32bitreg(spi_num, SYS_STATUS_ID, clear);  // Write status register to clear event bits we have seen
                ...
                } else  // Double buffer
                {
                    ...
                    dwt_readfromdevice(spi_num, SYS_STATUS_ID, 3, 1, &buff);
                    ...
                    if (dwt_checkoverrun(spi_num) == 0) {
                        // Toggle the host side Receive Buffer Pointer by writing one to the register
                        dwt_writetodevice(spi_num, SYS_CTRL_ID, SYS_CTRL_HRBT_OFFSET, 1, &hsrb);  // We need to swap RX buffer status reg (write one to toggle internally)
                    } else {
        ...
        else   // No LDE_DONE ?
        {
            ...
            dwt_rxreset(spi_num);  // Reset the RX
            dw1000local.wait4resp = 0;
            dw1000local.cdata.event = DWT_SIG_RX_ERROR;
            if (dw1000local.dwt_rxcallback != NULL) {
                dw1000local.dwt_rxcallback(&dw1000local.cdata);
 ...
 * no return value
 */
void dwt_setleds(uint8_t spi_num, uint8_t test) {
    uint8_t buf[2];

    if (test & 0x1) {
        // Set up MFIO for LED output
        dwt_readfromdevice(spi_num, GPIO_CTRL_ID, 0x00, 2, buf);
        buf[1] &= ~0x3C;  // clear the bits
        buf[1] |= 0x10;   // TW 0x14;
        buf[0] &= ~0xC0;
        buf[0] |= 0x40;
        dwt_writetodevice(spi_num, GPIO_CTRL_ID, 0x00, 2, &buf[0]);  // TW
        // dwt_writetodevice(GPIO_CTRL_ID,0x01,1,&buf[1]);//ORIG

        // Enable LP Oscillator to run from counter, turn on debounce clock
        dwt_readfromdevice(spi_num, PMSC_ID, 0x02, 1, buf);
        buf[0] |= 0x84;  //
        dwt_writetodevice(spi_num, PMSC_ID, 0x02, 1, buf);

        // Enable LEDs to blink
        buf[0] = 0x10;  // Blink period.
        buf[1] = 0x01;  // Enable blink counter
        dwt_writetodevice(spi_num, PMSC_ID, PMSC_LEDC_OFFSET, 2, buf);

    } else if ((test & 0x1) == 0) {
        // Clear the GPIO bits that are used for LED control
        dwt_readfromdevice(spi_num, GPIO_CTRL_ID, 0x00, 2, buf);
        buf[1] &= ~(0x14);
        dwt_writetodevice(spi_num, GPIO_CTRL_ID, 0x00, 2, buf);
    }

    // Test LEDs
    if (test & 0x2) {
        buf[0] = 0x0f;  // Fire a LED blink trigger
        dwt_writetodevice(spi_num, PMSC_ID, 0x2a, 1, buf);
...
void _dwt_enableclocks(uint8_t spi_num, int clocks) {
    ...
    dwt_readfromdevice(spi_num, PMSC_ID, PMSC_CTRL0_OFFSET, 2, reg);
    switch (clocks) {
        case ENABLE_ALL_SEQ: {
            reg[0] = 0x00;
            reg[1] = reg[1] & 0xfe;
    ...
    dwt_writetodevice(spi_num, PMSC_ID, PMSC_CTRL0_OFFSET, 1, &reg[0]);
    dwt_writetodevice(spi_num, PMSC_ID, 0x1, 1, &reg[1]);
 ...
 * no return value
 */
void _dwt_disablesequencing(uint8_t spi_num)  // Disable sequencing and go to state "INIT"
{
    _dwt_enableclocks(spi_num, FORCE_SYS_XTI);  // Set system clock to XTI

    dwt_write16bitoffsetreg(spi_num, PMSC_ID, PMSC_CTRL1_OFFSET, PMSC_CTRL1_PKTSEQ_DISABLE);  // Disable PMSC ctrl of RF and RX clk blocks
}
 ...
 * no return value
 */
void dwt_setdelayedtrxtime(uint8_t spi_num, uint32_t starttime) { dwt_write32bitoffsetreg(spi_num, DX_TIME_ID, 1, starttime); }  // end dwt_setdelayedtrxtime()
 ...
 */
int dwt_starttx(uint8_t spi_num, uint8_t mode) {
    ...
    if (mode & DWT_RESPONSE_EXPECTED) {
        ...
        dwt_writetodevice(spi_num, SYS_CTRL_ID, 0, 1, &temp);
    ...
    if (mode & DWT_START_TX_DELAYED) {
        ...
        dwt_writetodevice(spi_num, SYS_CTRL_ID, 0, 1, &temp);
        checkTxOK = dwt_read16bitoffsetreg(spi_num, SYS_STATUS_ID, 3);
        ...
        if ((checkTxOK & 0x0400) == 0)  // Transmit Delayed Send set over Half a Period away or Power Up error (there is enough time to send but not to power up individual blocks).
        {
            ...
            dwt_entersleepaftertx(spi_num, 0);
            dw1000local.wait4resp = 0;
 ...
 */
int dwt_checkoverrun(uint8_t spi_num) { return ((dwt_read16bitoffsetreg(spi_num, SYS_STATUS_ID, 2) & (SYS_STATUS_RXOVRR >> 16)) == (SYS_STATUS_RXOVRR >> 16)); }
 ...
 * no return value
 */
void dwt_forcetrxoff(uint8_t spi_num) {
    ...
    mask = dwt_read32bitreg(spi_num, SYS_MASK_ID);  // Read set interrupt mask
    ...
    dwt_write32bitreg(spi_num, SYS_MASK_ID, 0);  // Clear interrupt mask - so we don't get any unwanted events

    dwt_writetodevice(spi_num, SYS_CTRL_ID, 0, 1, &temp);  // Disable the radio

    // Forcing Transceiver off - so we do not want to see any new events that may have happened
    dwt_write32bitreg(spi_num, SYS_STATUS_ID, (SYS_STATUS_ALL_TX | SYS_STATUS_ALL_RX_ERR | SYS_STATUS_ALL_RX_GOOD));

    dwt_syncrxbufptrs(spi_num);

    dwt_write32bitreg(spi_num, SYS_MASK_ID, mask);  // Set interrupt mask to what it was
 ...
 * no return value
 */
void dwt_syncrxbufptrs(uint8_t spi_num) {
    uint8_t buff;
    // Need to make sure that the host/IC buffer pointers are aligned before starting RX
    dwt_readfromdevice(spi_num, SYS_STATUS_ID, 3, 1, &buff);
 ...
 * no return value
 */
void dwt_setrxmode(uint8_t spi_num, int mode, uint8_t rxON, uint8_t rxOFF) {
    uint16_t reg16 = RX_SNIFF_MASK & ((rxOFF << 8) | rxON);

    if (mode & DWT_RX_SNIFF) {
        // PPM_OFF 15:8, PPM_ON 3:0
        dwt_write16bitoffsetreg(spi_num, RX_SNIFF_ID, 0x00, reg16);  // Enable
    } else {
        dwt_write16bitoffsetreg(spi_num, RX_SNIFF_ID, 0x00, 0x0000);  // Disable
    }
}
 ...
 */
int dwt_rxenable(uint8_t spi_num, int delayed) {
    uint16_t temp;
    uint8_t temp1 = 0;
    dwt_syncrxbufptrs(spi_num);
    ...
    dwt_write16bitoffsetreg(spi_num, SYS_CTRL_ID, 0, temp);

    if (delayed)  // Check for errors
    {
        // uint32_t status1 = dwt_read32bitreg(SYS_STATUS_ID) ; // Read status register

        dwt_readfromdevice(spi_num, SYS_STATUS_ID, 3, 1, &temp1);

        if (temp1 & (SYS_STATUS_HPDWARN >> 24))  // If delay has not passed do delayed else immediate RX on
        {
            dwt_forcetrxoff(spi_num);                 // Turn the delayed receive off, and do immediate receive, return warning indication
 ...
 * no return value
 */
void dwt_setrxtimeout(uint8_t spi_num, uint16_t time) {
    uint8_t temp;

    dwt_readfromdevice(spi_num, SYS_CFG_ID, 3, 1, &temp);  // Read register

    if (time > 0) {
        dwt_write16bitoffsetreg(spi_num, RX_FWTO_ID, 0x0, time);

        temp |= (uint8_t)(SYS_CFG_RXWTOE >> 24);
        // OR in 32bit value (1 bit set), I know this is in high byte.
        dw1000local.sysCFGreg |= SYS_CFG_RXWTOE;

        dwt_writetodevice(spi_num, SYS_CFG_ID, 3, 1, &temp);
    } else {
        temp &= ~((uint8_t)(SYS_CFG_RXWTOE >> 24));
 ...
 * no return value
 */
void dwt_setpreambledetecttimeout(uint8_t spi_num, uint16_t timeout) { dwt_write16bitoffsetreg(spi_num, DRX_CONF_ID, DRX_PRETOC_OFFSET, timeout); }
 ...
 * no return value
 */
void dwt_setinterrupt(uint8_t spi_num, uint32_t bitmask, uint8_t enable) {
    decaIrqStatus_t stat;
    uint32_t mask;
    ...
    mask = dwt_read32bitreg(spi_num, SYS_MASK_ID);  // Read register
    ...
    dwt_write32bitreg(spi_num, SYS_MASK_ID, mask);  // New value
 ...
 * no return value
 */
void dwt_configeventcounters(uint8_t spi_num, int enable) {
    uint8_t temp = 0x0;  // disable
    // Need to clear and disable, can't just clear
    temp = (uint8_t)(EVC_CLR);  // Clear and disable
    dwt_writetodevice(spi_num, DIG_DIAG_ID, EVC_CTRL_OFFSET, 1, &temp);
 ...
 * no return value
 */
void dwt_readeventcounters(uint8_t spi_num, dwt_deviceentcnts_t *counters) {
    uint32_t temp;

    temp = dwt_read32bitoffsetreg(spi_num, DIG_DIAG_ID, EVC_PHE_OFFSET);  // Read sync loss (31-16), PHE (15-0)
    counters->PHE = temp & 0xFFF;
    counters->RSL = (temp >> 16) & 0xFFF;

    temp = dwt_read32bitoffsetreg(spi_num, DIG_DIAG_ID, EVC_FCG_OFFSET);  // Read CRC bad (31-16), CRC good (15-0)
    counters->CRCG = temp & 0xFFF;
    counters->CRCB = (temp >> 16) & 0xFFF;

    temp = dwt_read32bitoffsetreg(spi_num, DIG_DIAG_ID, EVC_FFR_OFFSET);  // Overruns (31-16), address errors (15-0)
    counters->ARFE = temp & 0xFFF;
    counters->OVER = (temp >> 16) & 0xFFF;

    temp = dwt_read32bitoffsetreg(spi_num, DIG_DIAG_ID, EVC_STO_OFFSET);  // Read PTO (31-16), SFDTO (15-0)
    counters->PTO = (temp >> 16) & 0xFFF;
    counters->SFDTO = temp & 0xFFF;

    temp = dwt_read32bitoffsetreg(spi_num, DIG_DIAG_ID, EVC_FWTO_OFFSET);  // Read RX TO (31-16), TXFRAME (15-0)
    counters->TXF = (temp >> 16) & 0xFFF;
    counters->RTO = temp & 0xFFF;

    temp = dwt_read32bitoffsetreg(spi_num, DIG_DIAG_ID, EVC_HPW_OFFSET);  // Read half period warning events
 ...
 * no return value
 */
void dwt_rxreset(uint8_t spi_num) {
    uint8_t resetrx = 0xe0;
    // Set RX reset
    dwt_writetodevice(spi_num, PMSC_ID, 0x3, 1, &resetrx);
 ...
 * no return value
 */
void dwt_softreset(uint8_t spi_num) {
    uint8_t temp[1] = {0};

    _dwt_disablesequencing(spi_num);
    //_dwt_enableclocks(FORCE_SYS_XTI); // Set system clock to XTI

    // Clear any AON auto download bits (as reset will trigger AON download)
    dwt_write16bitoffsetreg(spi_num, AON_ID, AON_WCFG_OFFSET, 0x0);
    // Clear the wake-up configuration
    dwt_writetodevice(spi_num, AON_ID, AON_CFG0_OFFSET, 1, temp);
    // Upload the new configuration
    _dwt_aonarrayupload(spi_num);

    // Reset HIF, TX, RX and PMSC
    dwt_readfromdevice(spi_num, PMSC_ID, 0x3, 1, temp);

    temp[0] &= 0x0F;
    dwt_writetodevice(spi_num, PMSC_ID, 0x3, 1, &temp[0]);
 ...
 * no return value
 */
void dwt_xtaltrim(uint8_t spi_num, uint8_t value) {
    uint8_t write_buf;

    dwt_readfromdevice(spi_num, FS_CTRL_ID, FS_XTALT_OFFSET, 1, &write_buf);

    write_buf &= ~FS_XTALT_MASK;

    write_buf |= (FS_XTALT_MASK & value);  // We should not change high bits, cause it will cause malfunction

    dwt_writetodevice(spi_num, FS_CTRL_ID, FS_XTALT_OFFSET, 1, &write_buf);
}
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_configcwmode(uint8_t spi_num, uint8_t chan) {
    uint8_t write_buf[1];
#ifdef DWT_API_ERROR_CHECK
    if ((chan < 1) || (chan > 7) || (6 == chan)) {
        return DWT_ERROR;  // validate channel number parameter
    ...
    // Enable RF PLL
    //
    dwt_write32bitreg(spi_num, RF_CONF_ID, RF_CONF_TXPLLPOWEN_MASK);  // Enable LDO and RF PLL blocks
    dwt_write32bitreg(spi_num, RF_CONF_ID, RF_CONF_TXALLEN_MASK);     // Enable the rest of TX blocks

    //
    // Configure TX clocks
    //
    write_buf[0] = 0x22;
    dwt_writetodevice(spi_num, PMSC_ID, PMSC_CTRL0_OFFSET, 1, write_buf);
    write_buf[0] = 0x07;
    dwt_writetodevice(spi_num, PMSC_ID, 0x1, 1, write_buf);

    // Disable fine grain TX seq
    dwt_write16bitoffsetreg(spi_num, PMSC_ID, PMSC_TXFINESEQ_OFFSET, PMSC_TXFINESEQ_DIS_MASK);

    write_buf[0] = TC_PGTEST_CW;

    // Configure CW mode
    dwt_writetodevice(spi_num, TX_CAL_ID, TC_PGTEST_OFFSET, TC_PGTEST_LEN, write_buf);
 ...
 * no return value
 */
void dwt_configcontinuousframemode(uint8_t spi_num, uint32_t framerepetitionrate) {
    uint8_t write_buf[4];
    ...
    // Configure TX clocks
    //
    _dwt_enableclocks(spi_num, FORCE_SYS_PLL);
    _dwt_enableclocks(spi_num, FORCE_TX_PLL);

    // Set the frame repetition rate
    if (framerepetitionrate < 4) {
        framerepetitionrate = 4;
    }
    dwt_write32bitoffsetreg(spi_num, DX_TIME_ID, 0, framerepetitionrate);
    //
    // Configure continuous frame TX
    //
    write_buf[0] = (uint8_t)(DIAG_TMC_TX_PSTM);
    dwt_writetodevice(spi_num, DIG_DIAG_ID, DIAG_TMC_OFFSET, 1, write_buf);  // Turn the tx power spectrum test mode - continuous sending of frames
}
 ...
 */
uint16_t dwt_readtempvbat(uint8_t spi_num, uint8_t fastSPI) {
    uint8_t wr_buf[2];
    uint8_t vbat_raw;
    uint8_t temp_raw;
    ...
    dwt_writetodevice(spi_num, RF_CONF_ID, 0x11, 1, wr_buf);

    wr_buf[0] = 0x0A;  // Enable TLD Bias and ADC Bias
    dwt_writetodevice(spi_num, RF_CONF_ID, 0x12, 1, wr_buf);
    ...
    wr_buf[0] = 0x00;
    dwt_writetodevice(spi_num, TX_CAL_ID, TC_SARL_SAR_C, 1, wr_buf);
    ...
    if (fastSPI == 1) {
        ...
        dwt_readfromdevice(spi_num, TX_CAL_ID, TC_SARL_SAR_LVBAT_OFFSET, 2, wr_buf);
    } else  // change to a slow clock
    {
        _dwt_enableclocks(spi_num, FORCE_SYS_XTI);  // NOTE: set system clock to XTI - this is necessary to make sure the values read are reliable
        // Read voltage and temperature.
        dwt_readfromdevice(spi_num, TX_CAL_ID, TC_SARL_SAR_LVBAT_OFFSET, 2, wr_buf);
        // Default clocks (ENABLE_ALL_SEQ)
        _dwt_enableclocks(spi_num, ENABLE_ALL_SEQ);  // Enable clocks for sequencing
 ...
 */
uint8_t dwt_readwakeuptemp(uint8_t spi_num) {
    uint8_t temp_raw;
    dwt_readfromdevice(spi_num, TX_CAL_ID, TC_SARL_SAR_LTEMP_OFFSET, 1, &temp_raw);
    return (temp_raw);
}
 ...
 */
uint8_t dwt_readwakeupvbat(uint8_t spi_num) {
    uint8_t vbat_raw;
    dwt_readfromdevice(spi_num, TX_CAL_ID, TC_SARL_SAR_LVBAT_OFFSET, 1, &vbat_raw);
    return (vbat_raw);
}
